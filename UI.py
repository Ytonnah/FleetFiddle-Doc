...
pg_heightADD = 100
root = Tk()
width,height = root.winfo_screenwidth(),(root.winfo_screenheight()+pg_heightADD)
...
paperframe = ttk.Frame(frm,width=1000,height=height,padding=0)
...
#make class for paper resolutions later as string
...
paper_res = "tstF" #replace array to dynamically change page resolution

'''you can place the widgets on canvas using create_window, figure it out'''

    #PAPERS ARE CREATED HERE
paper1 = paper.create_paper(270,100,"1160x2300",paperBg)
#paper2 = paper.create_paper(270,2500,paperBg)
...
#for outline
outline = Canvas(frm, width=200,height=400,background=Green1,highlightthickness=0,bd=0,relief=RIDGE)
outline.place(x=0,y=100)
...
class Alignment_F:
    ...
    '''
    states = ['CENTERED','AL-LEFT','AL-RIGHT']
...
class fontSizeTools:
    ...
    addFontSize.config(command=fontSizeF.add_Fsize)
